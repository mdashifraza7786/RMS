...
export async function getTotalRevenue(startDate: string, endDate: string): Promise<number> {
    ...
    try {
        // Calculate UTC boundaries to match DB timezone (assumed UTC)
        ...
        const [rows] = await connection.execute<RowDataPacket[]>(
            `SELECT COALESCE(SUM(total_amount), 0) as total_revenue
             ...
             AND generated_at >= DATE_SUB(?, INTERVAL ? MINUTE) 
             AND generated_at <= DATE_SUB(?, INTERVAL ? MINUTE)`,
            [`${startDate} 00:00:00`, offsetMinutes, `${endDate} 23:59:59`, offsetMinutes]
        );
